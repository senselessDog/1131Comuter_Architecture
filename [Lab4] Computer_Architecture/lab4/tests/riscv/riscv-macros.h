...
#define TEST_CMOV_SRC10_BYP( src1_nops_, src0_nops_, inst_,             \
    ...
    inst_ x2, x3, x4;                                                   \
    TEST_CHECK_EQ( x2, result_ );                                       \
// 測試 WAW (Write-After-Write) hazard 的巨集
#define TEST_WAW_HAZARD( fast_val_, slow_val1_, slow_val2_, result_ )    \
    li    x5, 0;                                                         \
    li    x2, slow_val1_;                                               \
    li    x3, slow_val2_;                                               \
    li    x4, fast_val_;                                                \
    mul   x5, x2, x3;         /* 慢指令：乘法 */                          \
    add   x5, x4, x0;         /* 快指令：加法 */                          \
    sw    x5, 0(x0);          /* memory操作確保完成 */                    \
    TEST_INSERT_NOPS(10);                                                \
    TEST_CHECK_EQ(x5, result_);                                          \

// 測試 Bypassing from ROB
#define TEST_BYP_FROM_ROB( fast_val_, slow_val1_, slow_val2_, result_ ,nops_)    \
    li    x5, 0;                                                         \
    li    x6, 0;                                                        \
    li    x2, slow_val1_;                                               \
    li    x3, slow_val2_;                                               \
    ...
    mul   x5, x2, x3;         /* 慢指令：乘法 */                          \
    add   x5, x4, x0;         /* 快指令：加法 */                          \
    TEST_INSERT_NOPS(nops_);      /* 確保add   x5, x4, x0 在ROB*/            \
    addi  x6,x5,0;            /*需bypass的instruction*/                  \
